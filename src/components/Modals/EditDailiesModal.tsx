...
interface Props {
    ...
    close: () => void;
    ign: string;
    localStorageDailies: any; // TODO
    setLocalStorageDailies: any; // TODO
}

const EditDailiesModal: React.FC<Props> = ({
    opened,
    close,
    ign,
    localStorageDailies,
    ...
    const onSubmit = handleSubmit(() => {
        ...
        setLocalStorageDailies([
            ...localStorageDailies,
            {
                ign: data?.CharacterName,
                img: data?.CharacterImgUrl,
                level: data?.Level,
                dailies: dailies,
            },
        ]);
                ...
                {dailyCategories.map((category) => {
                    return (
                        <div style={{ width: "40%" }}>
                            ...
                            {dailyList
                                .filter(
                                    (daily) => daily.type === category.category
                                )
                                .map((daily, index) => {
                                    return (
                                        <Controller
